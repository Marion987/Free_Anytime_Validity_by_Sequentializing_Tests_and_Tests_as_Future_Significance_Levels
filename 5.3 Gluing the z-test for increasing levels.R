...
k = 1 #set to 2, to see how many of the non-rejections of the glued test, happen after 2/3 of the data is observed (does will appear at value 520 in the plots)
...
glued_seq_test <- function(samples, comp_size, alpha){
  ...
  if(stopping_time + 1 > ceiling(comp_size/4)){
    return(seq_tests[ceiling(comp_size/4)])
  }
  ...
  if(stopping_time + 1 > ceiling(3*comp_size/4)){
    return(seq_tests[ceiling(3*comp_size/4)])
  }

...
test_3 <- function(samples, comp_size, alpha){
  ...
  for(i in 1:(ceiling(comp_size/2))){
    #compute the sequential test for all steps
    seq_tests[i] = seq_test(samples, 1, i, ceiling(comp_size/2), alpha)
  }
  #If we do not reach 0.3, return the comp_size + penalization to indicate a non-rejection
  if(length(which(seq_tests >= 0.3)) == 0) {
    return(0)
  }
  #else, stopping_time = index of first value that is >= 0.3
  stopping_time = which(seq_tests >= 0.3)[1]
  #if we stop at the end, return the last value of the current test
  if(stopping_time + 1 > ceiling(comp_size/2)){
    return(seq_tests[ceiling(comp_size/2)])
  }
  #compute the sequential test of level >=0.3 using the first 2/3 of the data, starting from where we stopped with the first test
  for(i in (stopping_time + 1):(ceiling(2*comp_size/3))){
    #compute the sequential test for all steps
    seq_tests[i] = seq_test(samples, stopping_time + 1, i, (ceiling(2*comp_size/3)) - (stopping_time + 1) + 1, seq_tests[stopping_time])
  }
  #If we do not reach 0.4, return the comp_size + penalization to indicate a non-rejection
  if(length(which(seq_tests >= 0.4)) == 0) {
    return(0)
  }
  #else, stopping_time = index of first value that is >= 0.4
  stopping_time = (which(seq_tests >= 0.4)[1])
  #if we stop at the end, return the last value of the current test
  if(stopping_time + 1 > ceiling(2*comp_size/3)){
    return(seq_tests[ceiling(2*comp_size/3)])
  }
  
  #For the rest of the data, compute the sequential test for level = value of seq. test at stopping time >= 0.4
  ...
  if(stopping_time + 1 > comp_size){
    return(seq_tests[comp_size])
  }
  
  
  #For the rest of the data, compute the sequential test for level = value of seq. test at stopping time >= 0.5
  for(i in (stopping_time + 1):(comp_size)){
    #compute the sequential test for all steps
    seq_tests[i] = seq_test(samples, stopping_time + 1, i, comp_size - (stopping_time + 1) + 1, seq_tests[stopping_time])
  }
  #If we do not reach 0.6, return the comp_size + penalization to indicate a non-rejection
  if(length(which(seq_tests >= 0.6)) == 0) {
    return(0)
  }
  #else, stopping_time = index of first value that is >= 0.6
  stopping_time = (which(seq_tests >= 0.6)[1])
  #if we stop at the end, return the last value of the current test
  if(stopping_time + 1 > comp_size){
    return(seq_tests[comp_size])
  }
  
  #For the rest of the data, compute the sequential test for level = value of seq. test at stopping time >= 0.6
  for(i in (stopping_time + 1):(comp_size)){
    #compute the sequential test for all steps
    seq_tests[i] = seq_test(samples, stopping_time + 1, i, comp_size - (stopping_time + 1) + 1, seq_tests[stopping_time])
  }
  ...
  #If we do not reach 0.8, return the comp_size + penalization to indicate a non-rejection
  if(length(which(seq_tests >= 0.8)) == 0) {
    return(0)
  }
  #else, stopping_time = index of first value that is >= 0.8
  stopping_time = (which(seq_tests >= 0.8)[1])
  #if we stop at the end, return the last value of the current test
  if(stopping_time + 1 > comp_size){
    return(seq_tests[comp_size])
  }
  
  #For the rest of the data, compute the sequential test for level = value of seq. test at stopping time >= 0.8
  for(i in (stopping_time + 1):(comp_size)){
    #compute the sequential test for all steps
    seq_tests[i] = seq_test(samples, stopping_time + 1, i, comp_size - (stopping_time + 1) + 1, seq_tests[stopping_time])
  }
  ...
  #The get a fast rejection, take the next 15 values and compute the sequential test for all of them
  for(i in (stopping_time + 1):(stopping_time + 15)){
    seq_tests[i] = seq_test(samples, stopping_time + 1, i, 15, seq_tests[stopping_time])
  }
  
  #If we do not reach 0.999, return the comp_size + penalization to indicate a non-rejection

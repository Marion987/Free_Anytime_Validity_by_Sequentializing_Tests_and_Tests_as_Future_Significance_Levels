...
test_3 <- function(samples, comp_size, alpha){
  ...
  #compute sequential test using the first 2/3 of the data points
  for(i in 1:(ceiling(2*comp_size/3))){
    #compute the sequential test for all steps
    seq_tests[i] = seq_test_23(samples, 1, i, ceiling(2*comp_size/3), alpha)
  }
  ...
  #For the rest of the data, compute the sequential test for level = value of seq. test at stopping time >= 0.3
  for(i in (stopping_time + 1):(comp_size)){
    #compute the sequential test for all steps
    seq_tests[i] = seq_test_23(samples, stopping_time + 1, i, comp_size - (stopping_time + 1) + 1, seq_tests[stopping_time])
  }
  ...
  #if there are no 20 data points left, return the last value of the current test
  if((stopping_time + 20) > comp_size){
    return(seq_tests[comp_size])
  }
  
  #The get a fast rejection, take the next 20 values and compute the sequential test for all of them
  for(i in (stopping_time + 1):(stopping_time + 20)){
    seq_tests[i] = seq_test_23(samples, stopping_time + 1, i, 20, seq_tests[stopping_time])
  }
  
  #reject at 0.999
  for(i in (stopping_time + 20):(comp_size)){
    #compute the sequential test for all steps
    seq_tests[i] = seq_tests[stopping_time + 20]
  }
